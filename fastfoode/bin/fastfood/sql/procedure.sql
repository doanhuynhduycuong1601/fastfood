...
as
begin
	Declare @ao table (NAM smallint,THANG smallint)
	insert into @ao select distinct YEAR(b.DATEPURCHASE),
	Month(b.DATEPURCHASE)
	from BILL b inner join PRODUCT p on b.IDPRODUCT = p.IDPRODUCT
			inner join STORE s on s.IDSTORE = p.IDSTORE
			where s.IDUSER = @userStore
	select (CAST(NAM as varchar) + '-' +
		IIF(THANG < 10, '0'+CAST(THANG as varchar),CAST(THANG as varchar))) as THANG
	from @ao
	ORDER BY THANG DESC
end
...
go
drop proc if exists proc_showProductDelete
go
create proc proc_showProductDelete @users varchar(50)
as
begin
	Declare @ID int = (select IDSTORE from STORE where IDUSER = @users)
	Select * from PRODUCT where IDSTORE = @ID and EXISTSS = 0
end
...
insert users(USERS,PASS,ROLES,EMAIL) values ('Customer12345','Customer123','user','nhduycuong16011601@gmail.com')
